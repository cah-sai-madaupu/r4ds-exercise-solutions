...
<!--html_preserve--><div id="htmlwidget-e5c8c404fe174e4c81bd" style="width:70%;height:355.968px;" class="grViz html-widget"></div>
...
However, not tail numbers appearing in `flights` from these carriers are missing from `planes`. I don't know how to reconcile the discrepancy.
...
First, I calculate all planes that have flown at least 100 flights.
...
Now, I will semi join the data frame of lights that have flown at least 100 flights to the data frame of flights to select the flights by those planes.
...
<img src="relational-data_files/figure-html/unnamed-chunk-45-1.png" width="70%" style="display: block; margin: auto;" />
...
The expression `anti_join(flights, airports, by = c("dest" = "faa"))` returns the flights that went to an airport that is not in the FAA list of destinations. Since the FAA list only contains
